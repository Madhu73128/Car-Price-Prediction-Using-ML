...
def authenticate(username, password):
    return username == "madhusudhan_3128" and password == "password@1435y"
...
def login_page():
    ...
    if st.button("Login"):
        if authenticate(username, password):
            ...
            time.sleep(3)  # Short delay for visual feedback
...
def main_page():
    ...
    # loading the dataset.
    car = pd.read_csv('cleaned_car_data_with_model.csv')
    ...
    # loading both models
    with st.spinner('Please ! Wait a Second While Logging in.......🔃'):
        #time.sleep(2)  # Delay for 5 seconds
        model_forest = model_loader("random_forest_regression_model.pkl")


    # # writing header
    #st.title('Car Price Predition Using MACHINE LEARNING')
    #st.markdown("<h2 style='text-align: center;'>🚗  Used Car Price Prediction 🚗</h2>", unsafe_allow_html=True)
    st.markdown("""
            <div style="text-align: center; color: indigo; font-size: 32px; font-weight: bold;">
    ...
    mileage = col1.number_input(label='Enter the car mileage (in miles)', help='How much the Car driven?')

    st.markdown("""
    <style>
        /* Style for slider track */
        .stSlider > div > div > div {
            background:orange; /* Change this color as needed */
        }
...
""", unsafe_allow_html=True)
    ...
    # 3. year integer
    year =  col1.slider('Enter the car manufacturing year (e.g., 2010)', 1980,2024,2005,help='The year when the car is manufactured.')


    # 4. brand integer
    brand_inp = col1.selectbox(label='Enter the car Brand(e.g., Audi)', options=brand_list, help='From which brand the car is belongs?') # passing the brand list
    brand = brand_dic[brand_inp] # converting the brand name to numerical encoding form



    # 6. engine type
    engine_type = col1.selectbox(label='Enter the car Engine Type(e.g., fuel)', options=engine_type_list, help='In which gas the car is run?')
    engine_type = engine_type_dic[engine_type]



    # 2. Engine volume integer
    engineV = col2.number_input(label='Enter the car engine volume. [e.g: 2.3]', max_value=6.4, help='Enter the volume of the car engine.')
    engineV = float(engineV)

    # 5. body type integer
    body_type = col2.selectbox(label='Enter the car body type', options=body_list, help='select the body type of the car')
    ...
    if brand_inp == 'Audi':
        ...
        st.image("Audi.jpg", use_column_width=True)
    ...
    if predict: 
            ...
            st.success(write)
            st.title("Prediction was Successful")
            st.balloons()
